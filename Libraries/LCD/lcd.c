...
static void LCD_WR_REG(uint16_t LCD_Reg)
{
	...
	//Command 읽어오기
        GPIO_SetBits(GPIOD, GPIO_Pin_15); //LCD_RD(1) Read_High
	GPIO_ResetBits(GPIOD, GPIO_Pin_13); //LCD_RS(0); command register
	GPIO_ResetBits(GPIOC, GPIO_Pin_6); //LCD_CS(0); Chip Select(Chip Enable)-> 0이 High, 1이 Low, H->L Falling Edge일때 LCD Chip 사용
	GPIO_ResetBits(GPIOD, GPIO_Pin_14); //LCD_WR(0);Write_Low
        
	GPIO_Write(GPIOE, LCD_Reg);
	// TODO implement using GPIO_ResetBits/GPIO_SetBits
        
	GPIO_SetBits(GPIOC, GPIO_Pin_6); //LCD_CS(1);//Low로 변경
	GPIO_SetBits(GPIOD, GPIO_Pin_14); //LCD_WR(1); Write High
}
...
static void LCD_WR_DATA(uint16_t LCD_Data)
{
	...
	GPIO_SetBits(GPIOD, GPIO_Pin_13); //LCD_RS(1);Data register
